...
**An enum is a contract too.** *Producer owns the contract* applies to enums, and `SignalOutcome` broke it by having two. A check-style worker reports `OK`, `FAILED` or `NO_RESULT`; the decision-engine publishes `OK`, `FAILED` or `NOT_EXECUTED`. The sets overlap without being interchangeable — a worker cannot conclude that no reply arrived, and the published event has no use for `NO_RESULT`, which it sends as a signal with no outcome and a reason. One enum spanning both producers was the union of the two sets, so every value the other producer could not legally use had to be rejected by a guard instead of being absent from the type. Split into `CheckOutcome`, owned by the worker, and `SignalOutcome`, owned by the decision-engine.
...
**decision-engine-internal types stay out.** Signal state, signal configuration and classification, and the correlation record are implementation details of the decision-engine and are not part of the shared module. The shared enums (`PaymentType`, `RiskLevel`, `CheckOutcome`, `DecisionResult`) are mirrored in the decision-engine domain, kept in alignment by `EnumCompatibilityTest`. A mirror earns its cost where domain logic dispatches on the value: `SignalConfig` keys each signal's applicable routes on `PaymentType`, and aggregation branches on `SignalOutcome` and `RiskLevel` (ADR-14). A producer's schema change therefore cannot silently redirect routing or a decision. Inert value objects carry no such risk and are not the subject of this rule.

**The intake message is not a contract crossing.** `EnrollmentEvent` is published and consumed by the decision-engine alone (ADR-13 §Ingress Inversion), so its payload arrives in the form it left in. The consume path therefore reads `EnrollmentData` directly instead of rebuilding the domain `EnrollmentCommand` from it: converting it back into a domain type protects against nothing, because no domain logic runs on it — only `paymentType` is read, and only to select the applicable signals. The mirroring above buys insulation where a foreign producer's schema reaches the aggregation, which is the signal results becoming `SignalState` (ADR-14), not on a message this service sent itself.
