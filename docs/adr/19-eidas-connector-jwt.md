# ADR-19: eIDAS Integration via Connector-Issued JWT, Not In-Pipeline Async Check
...
an enrollment can be submitted. The question is where in the architecture this verification sits
...
completes before the enrollment request is submitted. A dedicated eIDAS Connector
...
to the enrollment request. The decision-engine validates it offline at the PEP/PDP
...
the enrollment request alongside the other required fields.
