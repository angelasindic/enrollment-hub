...
**Decision traceability.** GDPR Art. 22 requires that automated decisions affecting a person be reconstructable after the fact. Two levels are involved and the system delivers them unequally. *How the decision was composed* is fully reconstructable: the settled signal map — each entry carrying its outcome, its risk level, or the reason it has neither — is persisted alongside the outcome on the correlation record and republished on the decision event, and trace context propagates across every service and message boundary (§6.3; ADR-14, ADR-17). *How a scoring signal reached its tier* is not: `GeoScoreResult` carries the neighbour counts and triggered radii behind a score, but only the tier is persisted, and the geo-index that produced it expires after 48 hours (ADR-12), so the measurement is not re-derivable from any retained input. Retaining the enrollment payload would not change that — which is why ADR-20 erases it after dispatch — and closing the gap means persisting the measurement at settle time (§10.1).
...
A control-by-control map of this section and §8.2 — implementation, the test that pins each control, and status — is in [security-controls.md](security-controls.md).
...
**Internal transport (not authenticated).** The layers above cover the user-facing path only. Between the decision-engine and its workers, RabbitMQ carries no per-message or per-service authentication: any client that can reach the broker can publish and consume any event. Broker network access is the trust boundary, and no user token crosses it. That holds while the broker is private to the deployment (§7.2); a shared, exposed, or multi-tenant broker is the trigger for hardening, scoped in ADR-03 §Internal trust posture.
...
- **Correlation store.** Holds enrollment workflow state and the decision of record. `original_request` (JSON) carries the full enrollment payload from intake until the decision event carrying it downstream has been confirmed by the broker — the engine has no other source for it, since the intake message is long acked and ADR-02 rules out asking the Account Service. After that confirm it has no reader: a stamped row is never re-claimed by the dispatch relay (ADR-17), so `PayloadRetentionJob` erases the column on its next pass, with no waiting period. From the confirm onward the decision event and its retention belong to the Account Service, which owns the queue it lands on (ADR-13 §Channel Ownership); a window here would hold personal data for a recovery this service neither performs nor owns. The engine's own intake and check-request queues are separate — it owns those and their DLQs. Where both services sit under the same controller that division assigns the work, not the obligation — a data-sharing or Art. 28 arrangement is a pre-production task. What survives is pseudonymous: ids, outcome, settled signals, timestamps; no name, phone, email, or address. That record is still personal data under Recital 26 — minimized, not anonymized — and it is retained rather than aged out, since it holds the only `decisionId → enrollmentId` bridge (ADR-17 withholds `enrollmentId` from the event). The deletion window for the stripped row remains open (§10.1). `decisionengine.payload.oldest.age` reports how long the oldest payload has been held; residual copies — in backups, in WAL, and in the superseded row version until autovacuum reclaims it — are bounded by those cycles rather than by the job (ADR-20 §Residual exposure).
- **Ephemeral storage (geocoding cache).** Address-derived coordinates under a 90-day TTL (`geocoding.cache.ttl`), longer than the geo-index window by design: the cache exists to spare Nominatim repeated lookups, and an address's coordinates do not change. The entry is keyed by a peppered HMAC-SHA256 digest of the normalized address and maps address → coordinates, not enrollment → address, so many enrollments collapse onto one entry and none is identifiable from it.
- **Minimization at the signal boundary.** Each check command carries only what its signal needs: `GeoScoreRequest` carries the shipping address and no identity, so geo-scoring never sees a name, email, or phone. `FraudCheckRequest` is the deliberate exception — it carries the full `EnrollmentData`, because the anticipated fraud signals (velocity, device fingerprinting, email-domain and IP-clustering analysis) correlate across identity, payment type, and both addresses. The current implementation is a stub that reads only `enrollmentId`, so that breadth is provisional and is re-examined when the real service lands (§10.1). Because each worker receives a slice, none of them can return the whole payload — which is why the correlation store holds it in between (ADR-20).
- **Prerequisite tokens.** Validated in memory, not persisted. Only the validation result (pass/fail) and failure reason are logged.
- **Address data never reaches the logs.** No log statement at any level carries an address, an address fragment, or coordinates derived from one. Where a line needs to identify the address it concerns, it logs the peppered cache digest (§8.2 keying scheme), which is stable across cache miss, store, and hit without being reversible. This closes the path that would otherwise defeat the digest one line away from where it is computed: geocoding is an HTTP call with the address in the query string, and both upstreams (Nominatim, libpostal) echo the request back in their error bodies — so request URIs are logged as path-only and error bodies are reduced to their status code. Diagnostic context is carried by the `enrollmentId` in the MDC, which is correlatable without being personal data on its own. This rule is load-bearing beyond the log files themselves: every service exports its logs over OTLP (`management.opentelemetry.logging.export.otlp`), so a line carrying an address would be copied into a store with its own retention that no cleanup job here can reach. The discipline is currently enforced by review rather than by a test.
...
| Prometheus | Metrics: scrapes the Micrometer Prometheus registry of all five services; evaluates the alert rules in `monitoring/prometheus/rules/`. Domain metrics include geocoding latency and cache hit rate (geo-scoring), DLQ depth, publish-failure counters, the outbox age, and per-signal settlement counters (decision-engine). |
...
| Late/redelivered result for an absent correlation row | A result whose row no longer exists finds no slot for the idempotency guard and instead throws `UnknownCorrelationException`, routed to the DLQ on first throw with no retry budget spent. A missing row is treated as an inconsistency for triage. Retention does not currently produce this condition — `PayloadRetentionJob` erases the payload but never deletes a row (ADR-20) — so the constraint binds any future deletion window instead: it must exceed the maximum plausible result-arrival window, or benign late results land in the DLQ as false inconsistencies | ADR-16, ADR-20, §10.1 |
...
| **Correlation-row deletion window** | Age out the stripped row after a window, or retain the pseudonymous decision record indefinitely. Any window is to be determined by the GDPR DPIA and the investigation workflow | The payload half is settled and implemented: `original_request` is erased after dispatch (ADR-20, §8.2). What remains open is whether the row itself is deleted. It carries no direct identifiers and holds the only `decisionId → enrollmentId` bridge, so retention is the current default — deletion would remove the record wanted in a dispute after the sensitive data is already gone. §8.8 binds any window chosen: it must exceed the maximum plausible result-arrival window. If a window is adopted, time-partitioning and dropping partitions is the implementation to reach for rather than a batched `DELETE` (ADR-20 §Options considered). |
| **Scoring-signal evidence** | Persist the measurement behind a score — neighbour counts, triggered radii, saturation — on the correlation row and/or the decision event; or accept that a scored decision records a conclusion without its basis | `GeoScoreResult` carries all three; `GeoScoreResultListener` reduces them to `SignalState.Scored(riskLevel)`, and nothing persists them on either side of the boundary. The geo-index expires after 48 hours (ADR-12), so the measurement cannot be re-derived later from any retained input. Until this is decided, §2.2's traceability claim covers how a decision was composed from its signals, not how a scoring signal reached its tier. |
| **`FraudCheckRequest` payload breadth** | Narrow the command to what the implementation reads, or keep the full `EnrollmentData` for the anticipated signal set | Deliberate exception to the per-signal minimization in §8.2. The contract is written for velocity, device-fingerprinting, email-domain, and IP-clustering analysis; the current stub reads only `enrollmentId`, so the full payload is transmitted and unused on both routes. Narrowing a shared contract (ADR-06) now to re-widen it later is churn, so the decision is bound to the Internal Fraud Detection workstream above. |
