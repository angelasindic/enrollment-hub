 ...
 * correlation id; the broker treats each as a fresh delivery). The intake
 * idempotency ledger in
 * {@link dev.sindic.enrollmenthub.decisionengine.service.EnrollmentIntakeService#processEnrollment}
 * — insert PENDING under the {@code enrollment_id} unique constraint, then transition
 * to COMPLETED after dispatch — makes the second delivery a no-op rather than a
 * PK-violation / eventual DLQ.
 *
 * <p>Under at-least-once delivery with an idempotent receiver (ADR-13
 * §Delivery & Concurrency Guarantees), the events queue may receive the {@code EnrollmentEvent}
 * one or two times depending on whether the second delivery arrives before or after the
 * first reaches COMPLETED. Either outcome is correct; the test asserts only
...
class EnrollmentIntakeRedeliveryIT extends BaseIntegrationTest {
    ...
    void redeliveredIntakeMessage_producesExactlyOneRow() {
        ...
        // would have raced through the intake ledger). Then re-assert
        // exactly one row — the enrollment_id PK guarantees this regardless.
        ...
        // delivery: the count can be 1 (the ledger was COMPLETED, so the second
        // delivery skipped re-dispatch) or 2 (the second delivery raced ahead of the
        // first reaching COMPLETED, so both dispatched). Asserting "at least one" keeps
        // the test deterministic without over-specifying the timing.
        await().atMost(Duration.ofSeconds(5)).untilAsserted(() -> {
