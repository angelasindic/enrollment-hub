-- Spring Authorization Server JDBC schema (SAS 7.0.x), adapted for PostgreSQL per the official
-- script headers shipped in the SAS jar: 'timestamp' -> 'timestamptz', 'blob' -> 'text'.
--
-- Persists registered clients, in-flight + issued authorizations (codes, access/refresh tokens),
-- and user consent so authorization state survives restarts and is shared across replicas (ADR-05).
-- Objects are created unqualified and land in the authorization_server schema via the connection
-- search_path (spring.datasource currentSchema) and flyway.default-schema.
